...
When two live agents contest the same symbol, Synapse now creates a deterministic, suggest-only coordinated proposal with one `keep` direction and one `adapt` direction. The proposal is transient, is delivered in `state.snapshot`, and becomes `resolved` only after both agents accept with `resolution.ack`; Synapse does not auto-edit code, kick agents, or change detection verdicts. A reject from either agent (a `resolution.ack` with `accept: false`), or a TTL timeout before both accept, voids the coordinated pair (`status: "voided"` with `voidReason: "rejected"` or `"timeout"`), reverts the conflict to `contested`, and surfaces an Owner escalation — the voided proposal in the live room state; a reject is also recorded as dismiss conflict feedback. The TTL is `SYNAPSE_RESOLUTION_TTL_MS` (default 5 minutes). This tracer covers the mechanical class only, with semantic classification and LLM-authored prose left to later slices. See [ADR-0002](docs/adr/0002-llm-resolution-mediator-suggest-only.md).
