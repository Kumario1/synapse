...
flowchart TB
  subgraph Dev["Developer machine"]
    ...
    MCP["MCP stdio adapter<br/>tools + context resources"]
    CLI["Synapse CLI commands<br/>join, up, doctor, check, report, demo, insights, pr-brief"]
...
sequenceDiagram
  ...
  participant Server
  participant Store
  participant Peer as v2 teammate daemon
  participant Legacy as v1/remote peer
  ...
  Server->>Store: persist entity row and capture StateOp
  Server-->>Peer: state.delta { repoId, seq, ops }
  Server-->>Legacy: state.snapshot fallback
  Peer->>Peer: applyStateOp; track lastStateSeq
  Peer->>Peer: on sequence gap, reconnect for fresh snapshot
```
...
### 2d208ce to d9c82b4 - Incremental Context Surfaces

```mermaid
flowchart LR
  Server["Server state"] --> Delta["v2 state.delta stream"]
  Server --> Snapshot["v1 snapshot fallback"]
  Daemon["Daemon cache"] --> Insights["synapse insights<br/>synapse_insights"]
  Daemon --> Brief["synapse pr-brief<br/>synapse_pr_brief"]
  Daemon --> Resources["MCP resources<br/>briefing, team-state, decisions, pr-brief"]
```

Recent work made Synapse's context surfaces more incremental and more agent-native. New clients can receive sequenced state operations instead of full snapshots, while CLI and MCP users can pull health summaries, PR handoffs, and resource snapshots from the same local cache. The v2 op vocabulary is deliberately the same domain language as the store layer: `upsertSession`, `upsertEditLock`, `deleteEditLock`, `deleteEditLocksForSession`, `upsertDelta`, `deleteDelta`, `appendPush`, `appendRepoEvent`, `upsertResolution`, `deleteResolution`, `appendSummary`, and `appendFeedback`.
...
### 7660dd6 - Merge pull request #65

Problem: Agent guidance generated by `synapse connect` could drift away from the real command and MCP tool surface. Solution: connected rules and instructions to `packages/protocol/src/command-catalog.ts` and added tests that compare generated guidance against the catalog. System impact: onboarding text now derives command references from the protocol package's shared catalog instead of duplicating them by hand.

### 0e1999d - Bind local services to loopback by default (#66)

Problem: Local daemon and server listeners could be exposed more broadly than needed during normal development. Solution: changed default bind hosts to `127.0.0.1`, while keeping explicit environment/configuration paths for deployments that intentionally listen on other interfaces. System impact: the default security posture became local-only without removing team/deployment escape hatches.

### 7b89fb9 - Plan 019: Constrain daemon file reads to the worktree (#67)

Problem: Daemon file operations such as checks and reports accepted paths that could escape the joined repository through traversal, absolute paths, or symlinks. Solution: added `resolveWorktreePath`, rejected unsafe inputs with `UnsafeWorktreePathError`, and routed daemon file reads through the helper. System impact: local analysis is constrained to the configured worktree, preserving the privacy boundary around raw code.

### 043b5c0 - Plan 020: Pin GitHub webhook repo ids to the signed payload (#68)

Problem: Webhook-derived repo identity could be influenced by request routing or parameters instead of the signed GitHub payload. Solution: pinned repo IDs to `repository.full_name` from the verified payload and updated persistence/briefing verifiers. System impact: GitHub events attach to the repository GitHub actually signed, reducing cross-repo spoofing and misattribution risk.

### 0da499a - fix server state load retry cache (#69)

Problem: A failed state load could poison the cache and cause later reads to keep seeing the failure. Solution: changed the state-load cache so failed loads are not retained as successful cache entries. System impact: transient store or startup errors can recover on the next read instead of wedging a repo room.

### 53e71d5 - fix cli session action flag handling (#70)

Problem: `synapse session --action` was sensitive to argument order, so valid commands could be ignored or parsed incorrectly. Solution: fixed session command parsing and tests so `--action start|heartbeat|end` is honored regardless of where it appears, with invalid actions rejected. System impact: scripted and human session updates became predictable.

### 271e107 - fix analyzer ts barrel symbol identity (#71)

Problem: TypeScript barrel re-exports could make dependency edges point at the re-exporting file's symbol identity instead of the defining file's identity. Solution: preserved defining-file symbol ids while resolving barrel exports. System impact: dependency conflict detection now follows the actual contract owner, avoiding missed or misleading warnings around index/barrel modules.

### 672c59c - Plan 024: Make Go analyzer failures visible in CI and package checks (#72)

Problem: Go analyzer failures could be hidden by optional sidecar behavior, making CI pass even when Go support was broken on machines with Go installed. Solution: changed verification so Go analyzer setup/build failures fail when Go is present, while still skipping only when Go is absent. System impact: Go support became a real package and CI contract instead of a best-effort path.

### fd2449c - fix(analyzers): time out stuck sidecar requests (#73)

Problem: A hung Python or Go analyzer sidecar request could block local analysis indefinitely. Solution: added request timeouts (`SYNAPSE_ANALYZER_REQUEST_TIMEOUT_MS`, default 5000ms), sidecar reset behavior, and recovery tests for both sidecars. System impact: analyzer failures are isolated and recoverable, keeping checks and reports from hanging on one stuck subprocess.

### 9dd501e - Plan 026: Align the documented and packaged Node runtime floor (#74)

Problem: The repo and packaged CLI did not clearly agree on the supported Node/npm floor. Solution: pinned the repository to Node `20.19.2` and npm `11.4.1`, and set package engines to require Node `>=20.19.0`. System impact: local development, CI, and published package installs now share one runtime baseline.

### d027206 - chore(release): consolidate package verification path (#75)

Problem: Package verification had overlapping entry points, which made it unclear which command was authoritative. Solution: made `scripts/build-package.mjs` and `verify:package` the canonical package checks, added destination support for staged builds, filtered Python cache artifacts, and kept `verify:npm-pack` as a compatibility alias. System impact: release validation became easier to invoke consistently without breaking older scripts.

### 2d208ce - Plan 028: Implement protocol v2 state.delta broadcast (#76)

Problem: Full state snapshots are heavier than necessary for clients that already have a warm cache, but older clients still need compatibility. Solution: bumped the protocol to v2 while keeping v1 supported, added optional `seq` on snapshots, emitted `state.delta` with `{ repoId, seq, ops }`, and taught daemons to apply `applyStateOp`, ignore stale or wrong-repo deltas, and reconnect on sequence gaps. System impact: new daemon clients update incrementally and detect ordering through sequence numbers, while mixed-version deployments remain supported. Verification: `verify:delta-broadcast`.

### 4b87c56 - feat(mcp): expose Synapse context resources (#77)

Problem: MCP clients could call tools, but they lacked standard resource URIs for passive context reads. Solution: exposed `synapse://briefing`, `synapse://team-state`, `synapse://decisions`, and `synapse://pr-brief` through the MCP adapter and connect guidance. System impact: agents can discover and read current Synapse context as MCP resources, not only through command-style tools.

### 6a883aa - Plan 030: Add local synapse insights (#78)

Problem: Users and agents needed a compact health view over feedback, sessions, and delta delivery, not just raw briefings. Solution: added CLI `synapse insights`, MCP `synapse_insights`, daemon support, catalog entries, and `verify:insights`. The response includes feedback totals, acted/dismissed counts, active sessions, unpushed deltas, active edit locks, `topRulesByFeedback`, `topConflictTargets`, and `recentFeedback`. System impact: local cache data now produces an operational summary of warning feedback, session activity, and protocol health.

### d9c82b4 - Plan 031: Add PR handoff briefing (#79)

Problem: PR handoffs required stitching together local session state and GitHub webhook history by hand. Solution: added CLI `synapse pr-brief --base <base> --head <head>`, MCP `synapse_pr_brief`, `synapse://pr-brief`, and `verify:pr-brief`. The briefing filters by base/head branch and combines active sessions, unresolved deltas, edit locks, recent pushes, GitHub PR/review/comment events, and cited context as Markdown or JSON. System impact: an agent taking over a PR can request one focused handoff artifact instead of reconstructing context from multiple tools.
