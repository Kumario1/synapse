...
> Ground truth: originally verified against `main` @ `e353296`; refreshed through
> `main` @ `7e9409c` after plans 017-031 landed.
...
| PR decision memory / repo event recall (C3 slice) | ✅ | GitHub PR/review/comment events feed briefings, memory, and `pr-brief` |
| Onboarding + passive MCP context (C4 slice) | ✅ | `synapse onboard`, MCP resources, and catalog-derived connect guidance |
| Go analyzer (D2) | ✅ | `packages/analyzer-go` (M12, PR #43) |
| Incremental `state.delta` broadcast (D3) | ✅ | Protocol v2 `state.delta` ops with snapshot fallback; `verify:delta-broadcast` |
| Adaptive severity (F1) | ✅ | M6 (PR #32); branch-aware M6.5 (PR #37) |
| Rename/barrel tracking (F5 slice) | ✅ | same-file rename deltas plus barrel re-export symbol identity |
| Local coordination insights | ✅ | `synapse insights` / `synapse_insights`; `verify:insights` |
| PR handoff briefing | ✅ | `synapse pr-brief` / `synapse_pr_brief` / `synapse://pr-brief`; `verify:pr-brief` |
...
| VS Code (E1), dashboard/web (M13/E2), Slack/external ingestion, remaining F4/F5 automation | ⏸ | dashboard/OAuth remain decision-gated; Slack and richer auto-resolution remain deferred backlog |
...
5. **Write + broadcast amplification**: M8 fixed the write side with per-entity store ops, and D3
   has now shipped protocol-v2 `state.delta` fan-out for local v2 sockets. Remote multi-instance
   fanout still re-baselines with snapshots; Redis op relay remains the future scaling decision.
...
- **D3 — Incremental `state.delta` broadcast.** Shipped in PR #76: protocol v2 sockets receive
  sequenced per-entity ops; v1 sockets and remote multi-instance fanout keep snapshot baselines.
  Remaining decision: whether/when to add Redis op relay for remote mutations.
...
- **G6 fuzzing/property tests**: malformed-source fuzz for both analyzers; `resolutionInputsHash`
  symmetry properties.
- **Slack/external ingestion**, **remaining F4 richer auto-resolution**, **D3 phase-2 Redis op
  relay**, **more languages/SCIP**, **E1 VS Code extension**, **E4 editor rules**.
...
M15 negotiation ─→ D3 delta broadcast ✅ ─→ Redis op relay (future decision)
```
...
- 2026-06-11 — **Roadmap complete (autonomous run).** Every non-decision-gated milestone in §4 is
  ...
  46/46, CI green on every PR. Later execution shipped D3's local v2 delta broadcast plus C3/C4/F5
  slices; remaining owner decisions are **D4** fold `Synapse/` into `apps/web` → M13 dashboard and
  **D1/M14** GitHub OAuth (re-scoped to the SaaS launch phase by the plan's own default — build
  when a hosted offering launches). Remaining deferred backlog (no milestone specs): Slack/external
  ingestion, richer auto-resolution, Redis op relay for remote deltas, more languages/SCIP, E1
  VS Code, E4 editor rules.

- 2026-06-13 — **Plans 017-031 complete.** Local safety defaults and repo boundaries are hardened
  (loopback binds, worktree file reads, signed webhook repo binding, recoverable state loads);
  CLI/analyzer/release gaps are closed (`session --action`, TS barrel identity, visible Go setup
  failures, Python/Go sidecar timeouts, Node 20.19.x + npm 11.4.1 package verification); and the
  agent-facing surface grew protocol-v2 deltas, MCP resources, local insights, and PR handoff
  briefings. Commit `7e9409c` followed plan 031 to fix the verifier fixture so its signed
  `repository.full_name` matches the local repo binding.
