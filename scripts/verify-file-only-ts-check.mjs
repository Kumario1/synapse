...
const worktreeRoot = await mkdtemp(join(tmpdir(), "synapse-file-only-ts-check-"));
...
try {
  ...
  const cleanCheck = await checkFileOnly(bobPort);
  ...
  assert.deepEqual(
    check.conflicts.map((conflict) => [conflict.rule, conflict.targetSymbol.raw]),
    [["same_symbol_unpushed", symbol]]
  );

  console.log("File-only TypeScript check verification passed:");
