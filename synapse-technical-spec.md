...
- **Owner dashboard read.** The cookie-authed `GET /auth/projects/state?repoId=<id>` path returns the claimed Project's `TeamState` to the Owner dashboard, including active sessions, edit locks, persisted live Reservations, mediator state, and repo activity. Reservations are rendered as read-only regions in the browser; the dashboard adds no new Owner mutation or daemon WebSocket command for them.
