...
- **Owner** — a human who has signed in with GitHub (via the GitHub App) and claimed one or more Projects. Owners are the *only* humans in the model; the people "on a session" are AI agents, never human teammates.
...
- **Reservation** — a session's persisted edit region from reported work: held root symbols plus dependency-neighbor symbols. Read-only in the dashboard and distinct from a Contract or Contract delta.
...
- **Live grid** — the cleaned-up dashboard grid shown when a real server is connected (`mode === "live"`). Same panels, no scripted narration.
